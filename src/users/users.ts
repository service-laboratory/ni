...
import { type PaginationParams, UsersServer } from "./server";
...
export class Users {
  ...
  constructor(mountElement: HTMLElement) {
    this.server = new UsersServer(0);
    this.pagination = JSON.parse(
      localStorage.getItem("pagination") || '{"page": 5, "limit": 1000 }',
    );
  ...
  loadUsers() {
    ...
    return this.server.loadUsers(this.pagination, this.searchQuery).then((data) => {
      this.data = data;
  ...
  renderUsers() {
    ...
    userTable.innerHTML = html.getUserRowsHTML(this.data.users, this.searchQuery);
  }
  ...
  initPaginationEvents() {
    this.mountElement.addEventListener("change", (event) => {
      if ((event.target as HTMLSelectElement).id === "select-page-count") {
        this.pagination.limit = Number((event.target as HTMLSelectElement).value);
        this.updateUsersAndPagination();
      }
    });

    this.mountElement.querySelector(".pagination")?.addEventListener("click", (event) => {
      const target = event.target as HTMLElement;
  ...
  initSearchEvents() {
    const searchInput = this.mountElement.querySelector("#search-input") as HTMLInputElement;
